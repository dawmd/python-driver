...
class TokenAwarePolicyTest(unittest.TestCase):
    ...
    def test_no_leader_routing_for_weak_read_consistency(self):
        """
    ...
    def test_no_leader_routing_for_default_weak_consistency(self):
        """
        When a statement leaves consistency_level unset it defers to the
        execution profile. The policy must resolve that *effective* level (here
        the default profile's LOCAL_ONE) and, finding it weak, skip the leader
        hint -- otherwise the common default-consistency read would hotspot the
        leader.
        """
        policy, leader, other_replica = self._make_leader_routing_setup(
            default_consistency_level=ConsistencyLevel.LOCAL_ONE)
        query = Statement(routing_key=b'\x00\x00\x00\x01', keyspace='ks', table='tbl')
        # consistency_level intentionally left as None.
        qplan = list(policy.make_query_plan(None, query))

        self.assertEqual(qplan[0], other_replica)
        self.assertEqual(qplan.count(leader), 1)

    def test_leader_routing_for_default_strong_consistency(self):
        """
        The effective-consistency fallback must read the *actual* default profile
        level, not assume it is weak. With a strong default (LOCAL_QUORUM) and an
        unset statement level, the leader is still preferred.
        """
        policy, leader, other_replica = self._make_leader_routing_setup(
    ...
    def test_leader_routing_for_lwt_at_weak_consistency(self):
        """
        An LWT is coordinated on the leader by its Paxos/Raft rounds regardless
        of its (learn-phase) consistency level, which commonly defaults to
        LOCAL_ONE. The leader must still be preferred for LWTs even at a weak
        consistency level, otherwise every default-consistency LWT would take an
        extra hop.
        """
        policy, leader, other_replica = self._make_leader_routing_setup()
        query = Statement(routing_key=b'\x00\x00\x00\x01', keyspace='ks', table='tbl')
