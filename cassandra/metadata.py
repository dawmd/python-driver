...
class SchemaParserV3(SchemaParserV22):
    ...
    def _get_scylla_keyspaces_consistency(self):
        ...
        consistency_by_ks = {}
        # Strong consistency is an optional routing optimization, so degrade
        # gracefully to eventual consistency on any failure rather than aborting
        # the whole schema refresh. The missing table/column case (older Scylla)
        # is already absorbed by _query_build_rows via expected_failures; this
        # broad guard covers the remaining server/connection/timeout errors.
        try:
